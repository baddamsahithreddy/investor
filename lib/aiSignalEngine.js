...
export async function generateSignalFromData(stocks) {
  ...
  for (const stock of stocks) {
    try {
      const [rsiData, volumeData, earningsImpact, priceAction, newsSentiment, sectorSentiment, insiderStatus] = await Promise.all([
        getRSI(stock.symbol, stock.priceHistory),
        getVolumeSpike(stock.symbol, stock.volumeData),
      ...
      signals.push({
        ...
        sectorSentiment,
        earningsImpact,
        reason: buildReason(volumeData.status, rsiData.value, newsSentiment, earningsImpact, priceAction.action),
      });
